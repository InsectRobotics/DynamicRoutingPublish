...
if __name__ == "__main__":
    ...
    env.seed(0)
    env.env._max_episode_steps = 100000
    ...
    dest = np.array([(0, 0), (0, 4), (4, 0), (4, 3)])
    dest_state = dest[:, 0] * 5 + dest[:, 1]

    # DRN = DynamicRoutingNet(number_of_states, dt=dt, default_connection_strength=1e-5,
    #                         max_connection_strength=1, learning_rate=0.001,
    #                         increase_threshold=0, decrease_threshold=0, init_leakage_conductance=1e-5,
    #                         default_leakage_conductance=1e-8, target_update_rate=0.1)
    ...
    for i_episode in range(max_number_of_episodes):  # run 20 episodes
        ...
        if map_only:
            taxi_row, taxi_col, pass_idx, dest_idx = env.decode(observation)
            location_state = taxi_row * 5 + taxi_col
        if map_only:
        ...
        while True:  # run until episode is done
            ...
            if exploration == "random":
                action = env.action_space.sample()
            if taxi_row == 3 and taxi_col == 1 and pass_idx == 2 and dest_idx == 0:
                pass
            location_state = taxi_row * 5 + taxi_col
            if map_only:
                target_state = dest_state[dest_idx]
                DRN.reset_target()
            ...
            if Info:
                print(
                    ...
                    % (i_episode, step, reward, adapted_reward, DRN.cumu_reward, np.argmax(DRN.state_code), action,
                       success_count) + str(
                        DRN.targets))
                print('new observation: %s, taxi_row: %s, taxi_col: %s, pass_idx: %s, dest_idx: %s' % (
                    observation, taxi_row, taxi_col, pass_idx, dest_idx))
                #
                # print('info', info)
                # print('Observation \n', observation)
                # print('DRN.potential_vector\n', DRN.potential_vector)
                # print('DRN.inject_currents\n', DRN.conservation_vector[:DRN.number_of_neurons])

            cumu_reward += reward
            DRN.record_heatmap()
            DRN.unsupervised_learning(fact_action=last_action, first_step=step==1)
            DRN.reinforcement_learning(reward=adapted_reward, done=done, if_target_state=reward >= 20,
            ...
            step_logger.pass_idx = pass_idx
            step_logger.dest_idx = dest_idx
            ...
            if done:
                ...
                if recording:
                    DRN.recording()
                    DRN.memory_maintenance()
                step_logger.memory_maintenance()
                ...
                # DRN.plot_graph(block=True)
                # DRN.plot_heatmap(decoder=env.decode, task=environment)
                # DRN.save_graph(path_dict['model'])
                # import networkx as nx
                # import os
                #
                # nx.readwrite.edgelist.write_edgelist(DRN.graph, path=os.path.join(path_dict['model_path'], 'graph.csv'))
                # numpy.unravel_index(DRN.current_matrix.argmax(), DRN.current_matrix.shape)
                break
